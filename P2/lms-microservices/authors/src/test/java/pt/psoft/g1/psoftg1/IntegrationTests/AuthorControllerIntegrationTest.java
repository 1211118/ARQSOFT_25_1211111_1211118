...
class AuthorControllerIntegrationTest {
    ...
    void shouldCreateAuthorSuccessfully() throws Exception {
        ...
        mockMvc.perform(post("/api/authors")
                ...
                .andExpect(jsonPath("$.name").value("Rowling"))
                .andExpect(jsonPath("$.bio").value("Author of the Harry Potter series"));
    }

    @Test
    void shouldFindAuthorByNumber() throws Exception {
        Author author = authorRepository.save(new Author("Agatha Christie", "Famous mystery writer", null));

        mockMvc.perform(get("/api/authors/{authorNumber}", author.getAuthorNumber())
                        .header("Authorization", "Bearer " + AUTH_TOKEN))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.name").value("Agatha Christie"))
                .andExpect(jsonPath("$.bio").value("Famous mystery writer"));
    }
